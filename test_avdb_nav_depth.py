...
episode 67: crystal_hot_sauce, 图距离 5.96m (之前同类 ≥6.5m 全部 max_steps 失败)
...
config = {
    ...
    'start_episode_idx': 3,   # 过滤后列表第3个 = episode 67 (图距离 5.96m, 之前失败)
    'agent_cfg': {
...
import glob
runs = sorted(glob.glob('logs/ObjectNav_avdb_depth_ep67_50step/0_of_1/*/'))
if runs:
    latest = runs[-1]
