...
class SimWrapper:
    ...
    def __init__(self, cfg):
        ...
        # Agent configuration (create this first)
        ...
        # 对于简单场景，不设置 scene_dataset_config_file
        if 'scene_config' in cfg and cfg['scene_config']:
            backend_cfg.scene_dataset_config_file = cfg['scene_config']
        backend_cfg.enable_physics = True
        # 使用 CPU 软件渲染（llvmpipe）
        # 注意: GPU 渲染在 WSL2 下有 EGL/CUDA 设备映射问题
        # 如需 GPU: 设置环境变量 CUDA_VISIBLE_DEVICES=0 EGL_PLATFORM=surfaceless
        # 并在独立脚本中测试通过后再启用
        ...
        try:
            ...
            # 强制重建 NavMesh，确保路径查找正常工作
            navmesh_settings = habitat_sim.NavMeshSettings()
            navmesh_settings.set_defaults()
            if self.sim.recompute_navmesh(self.sim.pathfinder, navmesh_settings):
                logging.info('NavMesh recomputed successfully!')
            else:
                logging.warning('NavMesh recomputation failed!')
            
        except Exception as e:
            logging.warning(f"Error initializing simulator: {e}")
            logging.info("Retrying with fallback configuration...")
            
            # Retry with simplified configuration
            backend_cfg2 = habitat_sim.SimulatorConfiguration()
            backend_cfg2.scene_id = cfg['scene_path']
            # 对于简单场景，不设置 scene_dataset_config_file
            if 'scene_config' in cfg and cfg['scene_config']:
                backend_cfg2.scene_dataset_config_file = cfg['scene_config']
            backend_cfg2.enable_physics = False
            # 使用 CPU 软件渲染（llvmpipe），绕过 EGL/CUDA 兼容性问题
            backend_cfg2.gpu_device_id = -1
            
            sim_cfg2 = habitat_sim.Configuration(backend_cfg2, agents)
            
            try:
                self.sim = habitat_sim.Simulator(sim_cfg2)
                logging.info('Simulator initialized (fallback mode)!')
                
                # 在 fallback 模式下也重建 NavMesh
                navmesh_settings = habitat_sim.NavMeshSettings()
                navmesh_settings.set_defaults()
                if self.sim.recompute_navmesh(self.sim.pathfinder, navmesh_settings):
                    logging.info('NavMesh recomputed in fallback mode!')
                else:
                    logging.warning('NavMesh recomputation failed in fallback mode!')
                
            except Exception as e2:
                logging.error(f"Fallback initialization also failed: {e2}")
                raise RuntimeError(f"Could not initialize simulator: {e2}")
    ...
    def get_path(self, path):
        ...
        :param path: Target position for pathfinding (can be MultiGoalShortestPath or ShortestPath).
        :return: The path found by the pathfinder.
        """
        try:
            # Handle both MultiGoalShortestPath and ShortestPath
            if hasattr(path, 'requested_ends'):
                # MultiGoalShortestPath - use the first goal
                end_pos = path.requested_ends[0] if len(path.requested_ends) > 0 else None
            elif hasattr(path, 'requested_end'):
                # ShortestPath
                end_pos = path.requested_end
            else:
                logging.error(f"Unknown path type: {type(path)}")
                return 1000
            
            if end_pos is not None:
                # Debug: Check navigability
                start_navigable = self.sim.pathfinder.is_navigable(path.requested_start)
                end_navigable = self.sim.pathfinder.is_navigable(end_pos)
                
                if not start_navigable:
                    logging.warning(f"Start position {path.requested_start} is NOT navigable!")
                if not end_navigable:
                    logging.warning(f"End position {end_pos} is NOT navigable!")
            ...
            logging.error(f"Error in get_path: {e}")
            return 1000
